...
#define strerror(n) ((unsigned)(n) < sys_nerr ? sys_errlist[(n)] : "unknown error")
#endif

/*************** Micro getopt() *********************************************/
#define OPTION(c, v) (_O & 2 && **v ? *(*v)++ : !c || _O & 4 ? 0 \
                                                             : (!(_O & 1) && (--c, ++v), _O = 4, c && **v == '-' && v[0][1] ? *++*v == '-' && !v[0][1] ? (--c, ++v, 0) : (_O = 2, *(*v)++) : 0))
#define OPTARG(c, v) (_O & 2 ? **v || (++v, --c) ? (_O = 1, --c, *v++) : (_O = 4, (char *)0) : (char *)0)
...
void terminate(int status)
{
    ...
    {
        if (set_term_param(port_fd, &saved_tty_parameters) < 0)
...
#define isalnumx(chr) ((((chr) >= '0') && ((chr) <= '9')) || (((chr) >= 'a') && ((chr) <= 'z')) || (((chr) >= 'A') && ((chr) <= 'Z')) || (chr) == '_')
